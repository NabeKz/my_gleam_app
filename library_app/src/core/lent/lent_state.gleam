...
pub fn calculate_lent_state(latest_event: option.Option(events.LentEvent)) -> LentState {
  case latest_event {
    option.Some(events.BookLendEvent(lend)) -> Lend(renter_id: lend.renter_id)
    option.Some(events.BookReturnedEvent(_)) -> Available
