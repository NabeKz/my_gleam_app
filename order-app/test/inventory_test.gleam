...
pub fn add_product_success_test() {
  let command = commands.AddProductToInventory(
    product_id: "prod-001",
      ...
      case events {
        [event] -> {
          case event {
            ...
            _ -> should.equal("Expected ProductAddedToInventory", "but got different event")
          }
        }
        _ -> should.equal("Expected 1 event", "but got " <> list.length(events) |> int.to_string)
      }
    }
    command_handlers.Failure(error) -> should.equal("Expected success", "but got: " <> error)
  }
}
...
pub fn add_product_already_exists_test() {
  ...
  let result = command_handlers.handle_add_product(
    Some(existing_item),
    command,
    test_date,
  )
  ...
  case result {
    ...
    command_handlers.Success(_) -> should.equal("Expected failure", "but got success")
  }
}

// ReceiveStock テスト
pub fn receive_stock_success_test() {
  let command = commands.ReceiveStock(
    product_id: "prod-001",
    quantity: 50,
  ...
  let result = command_handlers.handle_receive_stock(
    Some(existing_item),
    command,
    test_date,
  )
      ...
      case events {
        [event] -> {
          case event {
            ...
            _ -> should.equal("Expected StockReceived", "but got different event")
          }
        }
...
pub fn reserve_stock_success_test() {
  let command = commands.ReserveStock(
    product_id: "prod-001",
    quantity: 30,
  ...
  let result = command_handlers.handle_reserve_stock(
    Some(existing_item),
    command,
    test_date,
  )
      ...
      case events {
        [event] -> {
          case event {
            ...
            _ -> should.equal("Expected StockReserved", "but got different event")
          }
        }
        _ -> should.equal("Expected 1 event", "but got multiple events")
      }
      ...
      case events {
        [event] -> {
          case event {
            ...
            _ -> should.equal("Expected StockShortage", "but got different event")
          }
        }
        _ -> should.equal("Expected 1 event", "but got multiple events")
      }
...
pub fn inventory_service_integration_test() {
  ...
  // 1. 商品を追加
  let add_command = commands.AddProductToInventory(
    product_id: "prod-001",
    product_name: "Test Product",
    initial_quantity: 100,
  )
  let result1 = inventory_service.execute_command(service, add_command, test_date)
  
  case result1 {
    inventory_service.ServiceSuccess(updated_service) -> {
      // 2. 在庫を入庫
      let receive_command = commands.ReceiveStock(
        product_id: "prod-001",
        quantity: 50,
        received_from: "Supplier A",
      )
      let result2 = inventory_service.execute_command(updated_service, receive_command, test_date)
      
      case result2 {
        inventory_service.ServiceSuccess(updated_service2) -> {
          // 3. 在庫アイテムを取得して状態を確認
          let get_result = inventory_service.get_inventory_item(updated_service2, "prod-001")
          
          case get_result {
            inventory_service.ServiceSuccess(maybe_item) -> {
              case maybe_item {
                Some(item) -> {
                  should.equal(150, item.available_quantity) // 100 + 50
                  should.equal(0, item.reserved_quantity)
                  should.equal(150, item.total_quantity)
                  should.equal(value_objects.Available, item.status)
                  should.equal(2, item.version) // 2つのイベントが適用されている
                }
                None -> should.equal("Expected item", "but got None")
              }
            }
            inventory_service.ServiceFailure(error) -> should.equal("Expected success", "but got: " <> error)
          }
        }
        inventory_service.ServiceFailure(error) -> should.equal("Expected success", "but got: " <> error)
      }
